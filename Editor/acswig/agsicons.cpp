...
void ReplaceIconFromFile(const char *iconName, const char *exeName) {
  ...
  if ((iconHeader.idCount > MAX_ICONS_IN_FILE) || (iconHeader.idCount < 1)) {
    fclose(icoin);
    throw new AGSEditorException("Unable to replace icon: Too many icons within this icon file, or this may not be a valid .ICO file.");
  }
  
  if (iconHeader.idType != 1) {
    fclose(icoin);
    throw new AGSEditorException("Unable to replace icon: this is not a valid icon file");
  }
  ...
  if (hUpdate == NULL) {
    fclose(icoin);
    throw new AGSEditorException("Unable to load the custom icon: BeginUpdateResource failed");
  }
...
void ReplaceResourceInEXE(const char *exeName, const char *resourceName, const unsigned char *data, int dataLength, const char *resourceType)
{
  ...
  if (hUpdate == NULL) 
  {
    throw new AGSEditorException("Unable to replace resource: BeginUpdateResource failed");
  }
