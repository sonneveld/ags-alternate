...
static int ff_match(AL_CONST char *s1, AL_CONST char *s2)
{
   ...
   if (!data) {
      size = strlen(s2);
      data = (FF_MATCH_DATA*)_AL_MALLOC(sizeof(struct FF_MATCH_DATA) * size * 2 + 1);
      if (!data)
         return 0;
   }
...
int al_findfirst(AL_CONST char *pattern, struct al_ffblk *info, int attrib)
{
   ...
   ff_data = (FF_DATA*)_AL_MALLOC(sizeof(struct FF_DATA));
   if (!ff_data) {
      errno = ENOMEM;
      return -1;
   }
   ...
   if (!strpbrk(pattern, "?*")) {
      ...
      if (stat(pattern, &s) == 0) {
         /* get file attributes */
         actual_attrib = ff_get_attrib(alw_get_filename(pattern), &s);
         ...
         if ((actual_attrib & ~attrib) == 0) {
            ...
            strncpy(info->name, alw_get_filename(pattern),  sizeof(info->name));
            return 0;
         }
      }
   ...
   strcpy(ff_data->dirname, pattern);
  p = const_cast<char*>(alw_get_filename(ff_data->dirname));
