...
#include <ali3d.h>
  ...
  IGraphicsDriver* GetStubGraphicsDriver(GFXFilter *) {
      return NULL;
...
ALLEGRO_COLOR faded_out_palette[256];
...
public:
  ...
  virtual void UnInit();
  virtual void ClearRectangle(int x1, int y1, int x2, int y2, ALLEGRO_COLOR *colorToUse);
...
void ALSoftwareGraphicsDriver::ClearRectangle(int x1, int y1, int x2, int y2, ALLEGRO_COLOR *colorToUse)
{
  int color = 0;
  if (colorToUse != NULL) 
