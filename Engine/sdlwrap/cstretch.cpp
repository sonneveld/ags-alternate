   ...
   int masked)
{
   ...
   for (; y < dyend; y++, sy += syinc) {
      (*stretch_line)((uintptr_t)(dst->line[y] + dxofs), src->line[sy] + sxofs);
      if (yc <= 0) {
	 sy++;
	 yc += ycinc;
      }
 ...
 */
void alw_stretch_blit(BITMAP *src, BITMAP *dst, int sx, int sy, int sw, int sh,
 ...
 */
void alw_stretch_sprite(BITMAP *dst, BITMAP *src, int x, int y, int w, int h)
{
   assert(src);
   assert(dst);
