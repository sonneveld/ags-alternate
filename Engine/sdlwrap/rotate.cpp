 ...
 * pivot_*()
 */
void alw_pivot_sprite (BITMAP *bmp, BITMAP *sprite, int x, int y, int cx, int cy, fixed angle)
{
  assert(bmp);
  assert(sprite);

  _pivot_scaled_sprite_flip(bmp, sprite, x<<16, y<<16, cx<<16, cy<<16, angle, 0x10000, FALSE);
}
...
void alw_rotate_sprite (BITMAP *bmp, BITMAP *sprite, int x, int y, fixed angle)
{
  assert(bmp);
  assert(sprite);

  _pivot_scaled_sprite_flip(bmp, sprite, (x<<16) + (sprite->w * 0x10000) / 2,
